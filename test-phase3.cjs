...
(async()=>{
 ...
 await page.goto('file://'+path.join(__dirname,'index.html'));await page.waitForFunction(()=>window.HeroV3&&window.EquipmentV3&&window.AppBoot?.ready,{timeout:30000});
 ...
 await page.click('#menuToggle');await page.click('[data-menu="영웅"]');await page.click('[data-hero="su"]');await page.click('[data-hero-tab="장착 장비"]');
 ok('paperdoll renders eight slots',await page.locator('.paperdoll [data-v3-gear-slot]').count()===8);
 await page.click('#close');await page.click('#settings');ok('long progression record visible',await page.locator('.phase3-progress').count()===1);await page.click('#close');
 const result=await page.evaluate(()=>{
  ...
  Game.heroV3.setHeroUpgradeAmount(10);let before=Game.state.levels.su[0];Game.state.gold=1e9;Game.select('su');Game.upgrade(0);r.bulk=Game.state.levels.su[0]===before+10;Game.heroV3.setHeroUpgradeAmount(1);Game.action('exitTest');
