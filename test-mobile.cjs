...
(async()=>{
 ...
 await page.goto('file://'+path.join(__dirname,'index.html'));await page.waitForFunction(()=>window.MobileGame&&window.Phase3B&&window.Phase5&&window.AppBoot?.ready,{timeout:30000});
 ...
 ok('phase5 controls survive mobile modal',await page.locator('.phase5-settings').count()===1);
 ...
 const sw=fs.readFileSync(path.join(__dirname,'sw.js'),'utf8');ok('service worker caches latest phase files',['phase3b-progression.js','phase5-combat.js','phase5.css','mobile-app.css'].every(x=>sw.includes(x)));
